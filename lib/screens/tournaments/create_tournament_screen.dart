  ...
  Future<void> _createTournament() async {
    ...
    if (_hostPosition.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('주최자 포지션을 선택해주세요'), backgroundColor: AppColors.error),
      );
      return;
    }
    ...
    if (_selectedDate.isBefore(minimumStartTime)) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: const Text('내전 시작 시간은 현재 시간으로부터 최소 30분 이후여야 합니다'),
    ...
    try {
      ...
      if (tournamentId != null) {
        ...
        // `replace`를 사용하여 현재 화면을 상세 화면으로 교체합니다.
        // 이렇게 하면 뒤로가기 시 내전 목록으로 이동하여 UX가 개선되고,
        // 현재 화면이 dispose되므로 `_debugLocked` 오류도 해결됩니다.
        context.replace('/tournaments/$tournamentId');

      } else {
        // 생성 실패 시 로딩 상태를 해제하고 오류 메시지 표시
        if (mounted) {
